...
const UnifiedFurnitureSearch = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const filtered = products.filter(product => {
          ...
          const matchesRoomType = !filters.room_type || product.room_type === filters.room_type;
          
          return matchesQuery && matchesVendor && matchesCategory && matchesRoomType;
        });
        setProducts(filtered);
      }
    } catch (err) {
      // For demo purposes, do client-side search
      const filtered = products.filter(product => {
        return !searchQuery || 
          product.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          product.category.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleSaveCredentials = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Add to saved credentials locally for demo
      ...
      alert('Demo: Credentials would be saved securely in production');
      setShowCredentialsModal(false);
      setCredentials({ vendor_name: '', username: '', password: '' });
    }
  };
  ...
  const handleScrapeProducts = async () => {
    try {
      ...
      setLoading(true);
      alert('Demo: Product scraping would start for ' + savedCredentials.length + ' vendors');
      
      // Simulate loading new products
      setTimeout(() => {
        setLoading(false);
        alert('Demo: Products scraped successfully!');
      }, 2000);
