...
const AdvancedFFEFeatures = ({ project, onUpdate }) => {
  ...
  const bulkUpdateStatus = async (newStatus) => {
    try {
      ...
      const updatePromises = selectedIds.map(itemId =>
        fetch(`${(window.ENV?.REACT_APP_BACKEND_URL || window.location.origin) || window.location.origin}/api/items/${itemId}`, {
          method: 'PUT',
  ...
  const exportToPDF = async () => {
    try {
      ...
      const response = await fetch(`${(window.ENV?.REACT_APP_BACKEND_URL || window.location.origin) || window.location.origin}/api/export/pdf`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
