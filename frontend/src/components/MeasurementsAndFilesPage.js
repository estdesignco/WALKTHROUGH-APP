...
export default function MeasurementsAndFilesPage({ projectId }) {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#0F172A' }}>
      {/* NO HEADER - Will use tabs from parent */}
        ...
        {project?.rooms?.map((room) => {
          ...
          return (
            <div key={room.id} className="mb-6">
              {/* EXACT DESKTOP ROOM HEADER STYLE */}
              <div 
                className="px-4 py-2 text-[#D4C5A9] font-bold mb-4 cursor-pointer"
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="bg-white text-black px-3 py-1 rounded text-sm">{roomPhotos.length} photos</span>
                    {measuredPhotos.length > 0 && (
                      <span className="bg-[#FFD700] text-black px-3 py-1 rounded text-sm">📏 {measuredPhotos.length}</span>
                    )}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        measuredPhotos.forEach(async (photo) => {
              ...
              {/* PHOTOS GRID - EXACT DESKTOP STYLE */}
              {expandedRooms[room.id] && (
                <div className="bg-[#1E293B] border-2 border-[#B49B7E] p-4 mb-2">
                  {roomPhotos.length === 0 ? (
                    <div className="text-center py-8 text-[#B49B7E]">No photos in this room</div>
                  ) : (
                    <div className="grid grid-cols-6 gap-3">
                      {roomPhotos.map((photo) => (
                        <div
                          key={photo.id}
                          className="relative border-2 border-[#D4A574]/50 rounded overflow-hidden hover:border-[#D4A574] transition-all cursor-pointer"
                        ...
                        >
                          ...
                          <div className="absolute inset-x-0 bottom-0 bg-black bg-opacity-90 text-white text-xs p-1 text-center">
                            {photo.metadata?.has_measurements ? (
                              <div className="text-[#FFD700] font-bold">📏 {photo.metadata?.measurement_count}</div>
                            ) : (
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 bg-black bg-opacity-95 z-50 flex flex-col">
          <div className="bg-[#1E293B] p-4 border-b-2 border-[#D4A574]">
            <div className="flex justify-between items-center">
              <h3 className="text-2xl font-bold text-[#D4A574]">{selectedPhoto.metadata?.room_name || 'Photo'} {selectedPhoto.metadata?.has_measurements && `- ${selectedPhoto.metadata?.measurement_count} measurements`}</h3>
                ...
                <svg className="absolute top-0 left-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none" style={{ zIndex: 10 }}>
                  <defs>
                    {['#FFD700', '#FF6B6B', '#4ECDC4', '#95E1D3', '#F38181', '#AA96DA', '#FCBAD3', '#FFFFD2'].map((color) => (
                      <marker key={color} id={`v-${color.replace('#', '')}`} markerWidth="4" markerHeight="4" refX="3" refY="1.5" orient="auto"><polygon points="0 0, 4 1.5, 0 3" fill={color} /></marker>
                    ))}
                  </defs>
                  {selectedPhoto.metadata.measurements.map((m, i) => (
                    <line key={i} x1={m.x1} y1={m.y1} x2={m.x2} y2={m.y2} stroke={m.color || '#FFD700'} strokeWidth="0.3" markerEnd={`url(#v-${(m.color || '#FFD700').replace('#', '')})`} />
                  ))}
                </svg>
              )}
          ...
          <div className="bg-[#1E293B] p-4 border-t-4 border-[#D4A574]">
            <div className="flex gap-3 justify-center">
              <button onClick={() => exportToCanva(selectedPhoto)} className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold">🎨 EXPORT TO CANVA</button>
