      ...
      <div className="bg-gradient-to-r from-[#1E293B] to-[#0F172A] p-6 border-b-4 border-[#D4A574] shadow-2xl">
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            ...
            <p className="text-xl text-[#D4C5A9] mb-4">All photos with measurements and project documents</p>
          ...
          {photos.length === 0 ? (
            ...
            <div>
              ...
              <div className="flex gap-3 mb-4">
                <button
                  onClick={async () => {
                    if (!confirm(`Export all ${photos.length} photos?`)) return;
                    alert('📦 Exporting...');
                    for (let i = 0; i < photos.length; i++) {
                      await exportToCanva(photos[i]);
                ...
                <button
                  onClick={async () => {
                    ...
                    printWindow.document.write('<html><head><title>Measurement Report</title><style>body{font-family:Arial;padding:20px}h1{color:#D4A574}img{max-width:100%;margin:20px 0}.measurement{background:#f0f0f0;padding:10px;margin:5px 0;border-left:3px solid #D4A574}</style></head><body>');
                    printWindow.document.write('<h1>📏 Measurement Report</h1>');
                    printWindow.document.write(`<p>Project: ${projectId}</p><p>Total Photos: ${photos.length}</p><hr>`);
                    photos.forEach((photo, index) => {
                      printWindow.document.write(`<h2>Photo ${index + 1}: ${photo.metadata?.room_name || 'Unknown'}</h2>`);
                      printWindow.document.write(`<img src="${photo.photo_data}" />`);
                      printWindow.document.write(`<p><strong>Measurements:</strong> ${photo.metadata?.measurement_count || 0}</p>`);
                      if (photo.metadata?.measurements) {
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {photos.map((photo, index) => (
                  <div
                    ...
                    onClick={() => { setSelectedPhoto(photo); setShowPhotoDetail(true); }}
                  >
                    <img src={photo.photo_data} alt={photo.file_name} className="w-full h-40 object-cover" />
          ...
          {files.length === 0 ? (
            ...
            <div className="space-y-3">
              {files.map((file, index) => (
                <div key={file.id || index} className="bg-[#1E293B] border border-[#D4A574]/30 rounded-xl p-4 hover:border-[#D4A574] transition-all">
                  <div className="flex justify-between items-center">
                    ...
                    <button className="bg-[#D4A574] hover:bg-[#C49564] text-black px-4 py-2 rounded-xl font-bold">📥 Download</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showPhotoDetail && selectedPhoto && (
        <div className="fixed inset-0 bg-black z-50 flex flex-col">
          <div className="bg-[#1E293B] p-4 border-b-2 border-[#D4A574]">
            <div className="flex justify-between items-center">
              <h3 className="text-2xl font-bold text-[#D4A574]">📏 {selectedPhoto.metadata?.room_name || 'Photo'} - {selectedPhoto.metadata?.measurement_count || 0} measurements</h3>
              <button onClick={() => { setShowPhotoDetail(false); setSelectedPhoto(null); }} className="text-[#D4A574] text-3xl hover:text-red-400">✕</button>
            </div>
          </div>

          <div className="flex-1 p-4 flex items-center justify-center bg-black overflow-auto">
            <div className="relative">
              <img src={selectedPhoto.photo_data} alt={selectedPhoto.file_name} className="max-w-full max-h-[70vh] object-contain border-2 border-[#D4A574] rounded-xl" />
                ...
                <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none" style={{ zIndex: 10 }}>
                  <defs>
                    {['#FFD700', '#FF6B6B', '#4ECDC4', '#95E1D3', '#F38181', '#AA96DA', '#FCBAD3', '#FFFFD2'].map((color) => (
                      <marker key={color} id={`arrow-${color.replace('#', '')}`} markerWidth="4" markerHeight="4" refX="3" refY="1.5" orient="auto"><polygon points="0 0, 4 1.5, 0 3" fill={color} /></marker>
                    ))}
                  </defs>
                  {selectedPhoto.metadata.measurements.map((m, index) => (
                    <line key={index} x1={m.x1} y1={m.y1} x2={m.x2} y2={m.y2} stroke={m.color || '#FFD700'} strokeWidth="0.3" markerEnd={`url(#arrow-${(m.color || '#FFD700').replace('#', '')})`} />
                  ))}
                </svg>
              )}
              {selectedPhoto.metadata?.measurements && selectedPhoto.metadata.measurements.map((m, index) => (
                <div key={index} className="absolute" style={{ left: `${(m.x1 + m.x2) / 2}%`, top: `${(m.y1 + m.y2) / 2 - 4}%`, transform: 'translate(-50%, -100%)', zIndex: 20 }}>
                  <div className="bg-black bg-opacity-90 px-2 py-1 rounded text-sm font-bold border" style={{ color: m.color || '#FFD700', borderColor: m.color || '#FFD700' }}>{m.text || 'No measurement'}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#1E293B] p-4 border-t-4 border-[#D4A574]">
            <div className="flex gap-3 justify-center flex-wrap">
              ...
              <button onClick={() => { setShowPhotoDetail(false); setSelectedPhoto(null); }} className="px-8 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-xl font-bold text-lg">✕ CLOSE</button>
            </div>
          </div>
        </div>
      )}
