...
const RealDashboard = () => {
  ...
  const handleFullQuestionnaire = () => {
    window.open(`${window.location.origin}/customer/questionnaire`, '_blank');
  };

  const handleProjectClick = (project) => {
    if (project.id) {
      window.open(`${window.location.origin}/project/${project.id}/detail`, '_blank');
    } else {
      window.open(`${window.location.origin}/project/a332f2fb-a248-41db-9c50-f028430a6261/detail`, '_blank');
    }
  };
    ...
    <div className="min-h-screen" style={{
      ...
      <div className="flex flex-col items-center justify-start min-h-screen text-center px-4 pt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 max-w-6xl">
          <a 
            href={`${window.location.origin}/customer`}
            target="_blank" 
