...
const AdvancedFFEFeatures = ({ project, onUpdate }) => {
  ...
  const bulkUpdateStatus = async (newStatus) => {
    try {
      ...
      const updatePromises = selectedIds.map(itemId =>
        fetch(`https://app-finalizer-2.preview.emergentagent.com/api/items/${itemId}`, {
          method: 'PUT',
  ...
  const exportToPDF = async () => {
    try {
      ...
      const response = await fetch('https://app-finalizer-2.preview.emergentagent.com/api/export/pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
