...
const AdvancedFFEFeatures = ({ project, onUpdate }) => {
  ...
  const bulkUpdateStatus = async (newStatus) => {
    try {
      ...
      const updatePromises = selectedIds.map(itemId =>
        fetch(`https://code-scanner-14.preview.emergentagent.com/api/items/${itemId}`, {
          method: 'PUT',
  ...
  const exportToPDF = async () => {
    try {
      ...
      const response = await fetch('https://code-scanner-14.preview.emergentagent.com/api/export/pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
