...
const ExactChecklistSpreadsheet = ({
  ...
  const handleScrapeWebsite = async (url, itemId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Product data scraped successfully! Reloading...');
        window.location.reload();
      } else {
        console.error('❌ Scraping failed:', response.status);
        alert('Failed to scrape product data. Please check the URL.');
      }
    } catch (error) {
  ...
  if (!project || !project.rooms || project.rooms.length === 0) {
    return (
      <div className="text-center text-gray-400 py-8">
        ...
        <p className="text-sm mt-2">📋 CURATION PHASE - Link Scraping & Item Selection</p>
      </div>
    );
  }
            ...
            <table className="w-full border-collapse border border-gray-400">
              ...
              <thead>
                <tr>
                  ...
                  <th className="border border-gray-400 px-3 py-2 text-xs font-bold text-white" style={{ backgroundColor: '#8B4444' }}>COST</th>
                  ...
                  <th className="border border-gray-400 px-3 py-2 text-xs font-bold text-white" style={{ backgroundColor: '#8B4444' }}>ACTIONS</th>
                </tr>
              </thead>
                          ...
                          <React.Fragment key={category.id}>
                            ...
                            {isCategoryExpanded && category.subcategories?.map((subcategory) => (
                              <React.Fragment key={subcategory.id}>
                                  ...
                                  <tr key={item.id}>
                                    ...
                                    <td className="border border-gray-400 px-2 py-2 text-white text-sm">
                                      <input 
                                        ...
                                        className="w-full bg-transparent border-none text-white text-sm"
                                        placeholder="$0.00"
                                        defaultValue={item.cost ? `$${item.cost}` : ''}
                                      />
                                    </td>
                                    ...
                                    <td className="border border-gray-400 px-2 py-2 text-white text-sm">
                                      {item.link ? (
                                        ...
                                        <input 
                                          ...
                                          onBlur={(e) => {
                                            if (e.target.value) {
                                              console.log('🔗 Link added for scraping:', e.target.value);
                                            }
                                          }}
                                        />
                                      )}
                                    ...
                                    <td className="border border-gray-400 px-2 py-2 text-center">
                                      {item.link && (
                                        <button
                                          onClick={() => handleScrapeWebsite(item.link, item.id)}
                                          className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded text-xs"
                                          title="Scrape product data from link"
                                        >
                                          🔍 SCRAPE
                                        </button>
                                      )}
                                ...
                                {/* ADD ITEM BUTTON ROW */}
                                <tr>
                                  <td colSpan="9" className="border border-gray-400 px-6 py-2 bg-slate-900">
                                    <div className="flex justify-start items-center space-x-4">
                                      ...
                                      >
                                        ...
                                        <option value="Paint, Wallpaper, Hardware & Finishes">Paint & Finishes</option>
                                      ...
                                      <button
                                        onClick={() => {
                                          ...
                                          if (canvaLinks.length > 0) {
                                            ...
                                            alert('No Canva links found in this category.');
                                          }
                                        }}
                                      ...
                                      <button
                                        onClick={() => {
                                          // Teams integration for status updates
                                          console.log('📢 Sending Teams notification for category:', category.name);
                                          alert(`Teams notification sent for ${category.name} updates!`);
                                        }}
