...
const PerfectDashboard = () => {
  ...
  const handleFullQuestionnaire = () => {
    window.open('${process.env.REACT_APP_BACKEND_URL || window.location.origin}/customer/questionnaire', '_blank');
  };

  const handleProjectClick = (project) => {
    // Make projects clickable - open project detail page
    if (project.id) {
      window.open(`${process.env.REACT_APP_BACKEND_URL || window.location.origin}/project/${project.id}/detail`, '_blank');
    } else {
      window.open('${process.env.REACT_APP_BACKEND_URL || window.location.origin}/project/a332f2fb-a248-41db-9c50-f028430a6261/detail', '_blank');
    }
  };
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <a 
            href="${process.env.REACT_APP_BACKEND_URL || window.location.origin}/customer" 
            target="_blank" 
