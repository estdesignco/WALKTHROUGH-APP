...
const ExactFFESpreadsheet = ({ 
  ...
  const handleStatusChange = async (itemId, newStatus) => {
    ...
    try {
      const response = await fetch(`https://studio-assist-1.preview.emergentagent.com/api/items/${itemId}`, {
        method: 'PUT',
  ...
  useEffect(() => {
    const loadAvailableCategories = async () => {
      try {
        const backendUrl = "https://studio-assist-1.preview.emergentagent.com";
  ...
  const handleAddCategory = async (roomId, categoryName) => {
    ...
    try {
      ...
      const tempRoomResponse = await fetch(`https://studio-assist-1.preview.emergentagent.com/api/rooms`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (tempRoomResponse.ok) {
        ...
        if (matchingCategory) {
          ...
          const addResponse = await fetch(`https://studio-assist-1.preview.emergentagent.com/api/categories`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (addResponse.ok) {
            ...
            await fetch(`https://studio-assist-1.preview.emergentagent.com/api/rooms/${tempRoom.id}`, {
              method: 'DELETE'
            });
  ...
  const handleDragEnd = async (result) => {
    ...
    if (type === 'room') {
      ...
      try {
        const response = await fetch('https://studio-assist-1.preview.emergentagent.com/api/rooms/reorder', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    if (type === 'category') {
      ...
      try {
        ...
        const response = await fetch('https://studio-assist-1.preview.emergentagent.com/api/categories/reorder', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            room_id: roomId,
