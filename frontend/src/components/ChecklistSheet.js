...
const ChecklistSheet = () => {
  ...
  const scrapeCanvaBoard = async (itemId, canvaLink) => {
    try {
      ...
      const response = await fetch('https://spreadsheet-revamp.preview.emergentagent.com/api/scrape-product', {
        method: 'POST',
        ...
        <ExactChecklistSpreadsheet
          ...
          onDeleteRoom={(roomId) => {
            if (window.confirm('Delete this room?')) {
              fetch(`https://spreadsheet-revamp.preview.emergentagent.com/api/rooms/${roomId}`, {
                method: 'DELETE'
              }).then(() => {
                loadProject();
              });
        ...
        <AddRoomModal
          ...
          onSubmit={async (roomData) => {
            try {
              ...
              const response = await fetch('https://spreadsheet-revamp.preview.emergentagent.com/api/rooms', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
