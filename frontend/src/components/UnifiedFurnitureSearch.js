...
const UnifiedFurnitureSearch = () => {
  ...
  const [selectedProductForHouzz, setSelectedProductForHouzz] = useState(null);
  ...
  const handleHouzzAssign = (houzzData) => {
    console.log('Product assigned to Houzz:', houzzData);
    setShowHouzzModal(false);
    setSelectedProductForHouzz(null);
    setSuccess(`Successfully added ${houzzData.product.name} to Houzz Pro project!`);
  };
            ...
            {products.map((product) => (
              <div key={product.id} className={`bg-black/60 border rounded-lg p-4 hover:border-[#B49B7E]/40 transition-all duration-300 ${
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  <button
                    onClick={() => {
                      setSelectedProductForHouzz(product);
...
Copy this data to Houzz Pro clipper!
      ...
      {/* Houzz Integration Modal */}
      {showHouzzModal && selectedProductForHouzz && (
        <HouzzIntegrationModal
          product={selectedProductForHouzz}
          onClose={() => {
            setShowHouzzModal(false);
            setSelectedProductForHouzz(null);
          }}
          onAssign={handleHouzzAssign}
        />
      )}
    </div>
  );
